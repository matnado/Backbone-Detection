...
int tWorkersBusy = 0; //number of busy threads
std::vector<std::thread> tWorkers; //vector of threads
int tWorkersCount = 0;
bool tFinished = false;
bool tStartrow = false;
int numthreads;
unsigned long jcurr = 0; //currently computed element
...
std::mutex* counterMutex; //array of mutexes for each element in splitPrev
...
//	printf("moved node to pool [%p, %ld, %ld, %p]\n", node, node->indexnumber, node->numchilds, node->pathparent);
...
unsigned long searchlowerboundjaga(unsigned long start, unsigned long stop,
    ...
    unsigned long long &localbinsearchsteps) {
...
unsigned long searchupperboundjaga(unsigned long start, unsigned long stop,
    double searchvalue, unsigned long long &localbinsearchsteps) {
  unsigned long lo = start;
  unsigned long hi = stop;
  while (lo != hi) {
#ifdef STATISTICS
...
void threadComputeElement() {
  double sserr;
  while (!tFinished) {
    std::unique_lock<std::mutex> wqlk(multiMutex);
    while ((jcurr >= lineJmax && tWorkersBusy > 0) || !tStartrow) // used to avoid spurious wakeups
    {
    ...
    if (jcurr <= lineJmax && !tFinished) {
      //We can start working
      unsigned long j = jcurr;
      tWorkersBusy++; // increment counter of busy threads
      jcurr++; // set next computation to have done
      //release icurr for other processes
      wqlk.unlock();

      unsigned long long localbinsearchsteps = 0;
#ifdef STATISTICS
      unsigned long long localinnerloopcount = 0;
      unsigned long long localseedsteps = 0;
#endif
...
#endif //SEEDJAG
#ifdef SEEDOUR
        //upper row j-delta ... j+delta
        unsigned int delta=1;
        int left = j - delta - (i - 1);//left and right are not cell values but indices in splitPrev!
        int right = j - (i - 1);
      ...
      {
        wqlk.lock();
        splitCur[j - i]->pathparent = splitPrev[optsplit - i + 1];
        splitPrev[optsplit - i + 1]->numchilds++;
        wqlk.unlock();
      }
#else
      J[i][j] = optsplit;
...
#ifdef STATISTICS
      {
        std::lock_guard<std::mutex> statisticslk(statisticsMutex);
        seedsteps+=localseedsteps;
        innerloopcount+=localinnerloopcount;
        binsearchsteps+=localbinsearchsteps;
      }
#endif
    } else if (tFinished) {
      wqlk.unlock();
  ...
  {
    std::lock_guard<std::mutex> locker(multiMutex);
    tWorkersCount--;
  }
}
...
void computeMatrices() {
  ...
  for (int twc = 0; twc < numthreads; twc++) {
    tWorkersCount++;
    tWorkers.push_back(std::thread(threadComputeElement));
  }
  for (i = 2; i <= c; i++) {
    ...
    //Wait that workers have t_finished
    ...
    // TODO: test if initialization until the next gap is sufficient!
    // TODO: test if initialization once at beginning is enough, we reuse the array and therefore max should not be modified
  ...
  //notify all waiting threads to finish
  tFinished = true;
  while (tWorkersCount > 0) {
    workerStart.notify_one();
  }

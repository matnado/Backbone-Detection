...
def links_TFM(Net, myfile_links, myfile_edgelist, alpha, Bonferroni_corr, dt):	
	...
	myfile_links.write(str(dt)+'	'+str(len(edTFM))+'\n')
...
def links_EADM(Net, myfile_links, myfile_edgelist, alpha, Bonferroni_corr, dt):	
	...
	myfile_links.write(str(dt)+'	'+str(len(edEADM))+'\n')
...
def Backbone_detection(path_in = 'DATASET/Example_PrimarySchool/', name_file='primaryschool.csv', sep='\t', dt=[20., 60.*1., 60.*5., 60.*15.], multiedges = ['no'], directory_out = 'Output', column_time=0, remove_nights='yes', alpha = 0.01, Bonferroni_corr = 'yes', model = 'EADM'):
	...
	for i in range(len(dt)):
		...
		if model=='EADM' or model=='EADM_Accuracy':
			Net.Bayesian(x)
		...
		if model=='EADM_Accuracy':
			...
			links_EADM(Net, myfile_links, myfile_edgelist, alpha, Bonferroni_corr, dt[i])
		elif model=='EADM':
			...
			links_TFM(Net, myfile_links, myfile_edgelist, alpha, Bonferroni_corr, dt[i])
		else:
			sys.exit('Three are the acceptable keywords: TFM, SVN, EADM, or EADM_Accuracy. In the ReadMe file we explain their meaning')
